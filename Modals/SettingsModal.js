...
function SettingsModal({ onClose }) {
    ...
    if (stateData) {
      ...
      if (parsedState.length === 1 && !parsedState[0].tasks.length) {
        ...
        localStorage.setItem(
          "task-board-state",
          JSON.stringify([getEmptyGroup("No Status", false)])
        );
  ...
  return (
    <Modal
      show
      onClose={onClose}
      modalClass={s.modalClass}
      modalContentClass={s.modalContentClass}
    >
      <div className={s.container}>
        <div className={s.themeContainer}>
          ...
          <div className={s.theme} onClick={setDefaultTheme}>
            <div
              name="light"
              className={`${s.lightTheme} ${
                activeTheme === "light" && s.active
              }`}
            />
            <div
              name="dark"
              className={`${s.darkTheme} ${activeTheme === "dark" && s.active}`}
            />
          </div>
        </div>
        <hr />
        <div className={s.resetBoard}>
          ...
          <Tooltip tooltipText="Delete all tasks and groups" placement="top">
            <button className={s.dangerBtn} onClick={resetBoard}>
              Click here 😱
            </button>
          </Tooltip>
        </div>
